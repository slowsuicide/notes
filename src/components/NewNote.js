  ...
  saveNote() {
    ...
    if (this.noteFuncType === ACTION_VALUES.ACTION) {
      this.props.onChangeNote((notes) => {
        return (notes.concat({ ...JSON.parse(JSON.stringify(NOTE_MAIN_TEMPLATE)), id: notes.length + 1,header:this.header }));
      });
    }
    if (this.noteFuncType === ACTION_VALUES.SUBACTION) {
        this.props.onChangeNote((notes) => {
            ...
            if(el.id===this.props.parent.id)  
                el.subnotes.push({ ...JSON.parse(JSON.stringify(NOTE_DETAIL_TEMPLATE)),parentId:el.id, id: el.subnotes.length + 1,header:this.header })
            return el
            })
          });
    }
